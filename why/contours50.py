...
from numpy import uint8, zeros, array
...
def contours(src, dst):
    ...
    for o_idx,o_fr in enumerate(video_frames(src, height=None)):
        pframes.insert(0, o_fr)
        if len(pframes) > NFR:
            pframes.pop()
        elif len(pframes) < NFR:
            continue
        idx = o_idx - NFR + 1
        fr = array(pframes).mean(axis=0)
